...
extern "C" __declspec(dllexport) DWORD ThreadReader(LPVOID lpParam)
{
	...
	{
		HANDLE fileHandle = CreateFile((LPCSTR)(readerInfo.info->CurDir + *iterFileList).c_str(), GENERIC_READ,
...
extern "C" __declspec(dllexport) DWORD ThreadWriter(LPVOID lpParam)
{
	...
	while (true)
	{
		...
		cout << writerInfo.info->CurDir + writerInfo.outputFile << endl;
		HANDLE fileHandle = CreateFile((LPCSTR)(writerInfo.info->CurDir + writerInfo.outputFile).c_str(), GENERIC_WRITE,
